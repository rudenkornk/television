...
__tv_path_completion() {
    ...
    while true; do
        if [[ -z "$dir" || -d "$dir" ]]; then
            ...
            matches=$(
                tv "$dir" --autocomplete-prompt "$lbuf" --no-status-bar --input "$leftover" < /dev/tty | while IFS= read -r item; do
...
tv_shell_history() {
    ...
    output=$(tv bash-history --no-status-bar --input "$current_prompt")
